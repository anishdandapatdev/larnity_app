...
class _AddEventState extends ConsumerState<AddEvent> {
  ...
  Future<void> _saveEvent() async {
    ...
    try {
      ...
      if (_imageFile != null) {
        try {
          ...
          imageUrl = await storageService.uploadFile(
            bucket: StorageBucket.eventMedia,
            path: storagePath,
            file: _imageFile!,
          );
        } catch (storageError) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  "Warning: Storage bucket not found. Adding event without cover image.",
                ),
      ...
      // 3. Construct Model
      final isOnline = _selectedLocationType != 'address';
      final event = EventModel(
        ...
        endAt: endAt,
        isOnline: isOnline,
        location: !isOnline ? link : null,
        meetingUrl: isOnline ? link : null,
      );

      // 4. Call Provider
      await ref
          .read(eventProvider(widget.groupId).notifier)
          .createEvent(
            event: event,
            successCallBack: () {
              if (!mounted) return;
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      filled: true,
                      fillColor: AppColors.darkBgContainer,
                      hintText: _selectedLocationType == 'address'
                          ? "Enter address"
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppButton(
                    ...
                    label: _isSaving ? "Saving..." : AppStrings.addEvent,
